...
export class Store {
  ...
  #eventRow(id) { const row = this.#db.prepare(`SELECT e.id cursor,e.ticket_id,e.project,e.type,e.actor,e.device_id,e.message,e.metadata,e.created_at,a.role_id actor_role,d.name machine FROM events e LEFT JOIN actors a ON a.id=e.actor LEFT JOIN devices d ON d.id=e.device_id WHERE e.id=?`).get(id); return { ...row, metadata: parseMetadata(row.metadata) }; }
  ...
  async claimNext({ device: deviceId, actor: legacyId, session_capability: capability } = {}) { return this.#transaction(() => { const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const sessionId = this.#sessionCapability(capability, device.id).id; const ticket = this.#next(device); return ticket ? this.#claim(ticket, device, sessionId) : null; }); }
