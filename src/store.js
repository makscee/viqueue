...
export class Store {
  ...
  async claimNext({ device: deviceId, actor: legacyId, session_capability: capability, project } = {}) { const projectKey = project == null ? null : this.#requiredProject(project); return this.#transaction(() => { const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const sessionId = this.#sessionCapability(capability, device.id).id; const ticket = projectKey === null ? this.#nextExactAssigned(device) : this.#next(device, projectKey); return ticket ? this.#claim(ticket, device, sessionId) : null; }); }
