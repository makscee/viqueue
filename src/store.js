...
export class Store {
  ...
  #sourceLifecycle(id) { const submitted=this.#db.prepare("SELECT metadata,created_at FROM events WHERE ticket_id=? AND type='submitted' ORDER BY id DESC LIMIT 1").get(id),bundle=parseMetadata(submitted?.metadata)?.review_bundle; const fact = (name,type,absent,present) => { const event=this.#db.prepare('SELECT metadata,created_at FROM events WHERE ticket_id=? AND type=? ORDER BY id DESC LIMIT 1').get(id,type),metadata=parseMetadata(event?.metadata),reported=bundle?.source?.[name]; return event?{status:present,reference:metadata?.reference??null,updated_at:event.created_at}:{status:reported?.status??absent,reference:reported?.reference??null,updated_at:reported?submitted.created_at:null}; }; return { review:fact('review','review_recorded','not-reviewed','reviewed'),merge:fact('merge','merge_recorded','not-merged','merged') }; }
