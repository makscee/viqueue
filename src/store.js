...
export class Store {
  ...
  async claimNext({ project: rawProject, actor: rawActor }) { return this.#transaction(() => { const actor = this.#actor(rawActor); const ticket = this.#next(rawProject, actor); return ticket ? this.#claim(ticket, actor) : null; }); }
  ...
  async claim(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (ticket.state !== 'open' || ticket.claim || ticket.archived_at !== null || ticket.deleted_at !== null) throw new DomainError(409, 'ticket_unavailable', 'ticket is not available to claim'); if (!this.#eligible(ticket, actor)) throw new DomainError(409, 'assignee_ineligible', 'actor is not eligible for this assignment'); return this.#claim(ticket, actor); }); }
