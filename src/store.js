...
export class Store {
  ...
  async editTicket(id, changes = {}) { return this.#transaction(() => {
    const ticket = this.#ticket(id); const row = this.#row(id); let title = ticket.title; let body = ticket.body; let assignee = ticket.assignee; let project = ticket.project; let number = row.number;
  ...
  async setTicketState(id, { state, actor: rawActor }) { return this.#transaction(() => {
    const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (actor.kind !== 'human') throw new DomainError(403, 'human_required', 'a human actor is required');
