...
export class Store {
  ...
  #session(raw, { required = true } = {}) { if (raw == null && !required) return null; if (typeof raw !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:-]{15,199}$/.test(raw)) throw new DomainError(400, 'invalid_session_id', 'an opaque active Pi session id is required'); return raw; }
  #next(device) { const row = this.#db.prepare(`SELECT t.*,NULL claim_id,NULL claim_actor,NULL claim_device_id,NULL claim_session_id,NULL claim_machine,NULL generation,NULL claimed_at,(SELECT COUNT(*) FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open') unresolved_blockers,0 open_questions FROM tickets t WHERE t.state='open' AND t.board_state='Open' AND t.assignment='Agent' AND t.archived_at IS NULL AND t.deleted_at IS NULL AND NOT EXISTS(SELECT 1 FROM claims c WHERE c.ticket_id=t.id AND c.released_at IS NULL) AND NOT EXISTS(SELECT 1 FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open') ORDER BY t.board_order DESC,t.id LIMIT 1`).get(); const ticket = row ? this.#publicTicket(row) : null; return ticket && this.#claimable(ticket, device) ? ticket : null; }
  #claim(ticket, device, sessionId = null) { if (this.#db.prepare('SELECT 1 FROM claims WHERE device_id=? AND released_at IS NULL').get(device.id)) throw new DomainError(409, 'machine_already_claimed', 'this machine already has an active claim'); const generation = Number(this.#db.prepare('SELECT COALESCE(MAX(generation),0)+1 generation FROM claims WHERE ticket_id=?').get(ticket.id).generation); const token = randomBytes(32).toString('base64url'); const claimId = randomUUID(); this.#db.prepare('INSERT INTO claims(claim_id,ticket_id,actor,generation,token_hash,claimed_at,device_id,session_id) VALUES(?,?,?,?,?,?,?,?)').run(claimId, ticket.id, device.actor_id, generation, hash(token), this.#now(), device.id, sessionId); this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(this.#now(), ticket.id); this.#event(ticket.id, ticket.project, 'claimed', device.actor_id, null, { session_bound: sessionId !== null, machine: device.name }, device.id); return { ticket: this.#ticket(ticket.id), claim_token: token }; }
  async next({ device: deviceId, actor: legacyId } = {}) { const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); return this.#next(device); }
  async claimNext({ device: deviceId, actor: legacyId, session_id: rawSession }) { return this.#transaction(() => { const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const sessionId = this.#session(rawSession, { required: false }); const ticket = this.#next(device); return ticket ? this.#claim(ticket, device, sessionId) : null; }); }
  ...
  async claim(id, { device: deviceId, actor: legacyId, session_id: rawSession } = {}) { return this.#transaction(() => { const ticket = this.#ticket(id); const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const sessionId = this.#session(rawSession, { required: false }); if (sessionId !== null && this.#next(device)?.id !== ticket.id) throw new DomainError(409, 'ticket_ineligible', 'a session may claim only the first eligible global ticket'); if (!this.#claimable(ticket, device)) throw new DomainError(409, 'ticket_ineligible', 'ticket is not eligible for this paired worker'); return this.#claim(ticket, device, sessionId); }); }
  #authority(id, { claim_id: claimId, actor, device, session_id: rawSession, generation, claim_token: token }) { const current = this.#db.prepare('SELECT * FROM claims WHERE ticket_id=? AND released_at IS NULL').get(id); const candidate = typeof token === 'string' ? hash(token) : Buffer.alloc(0); const stored = current?.token_hash ? Buffer.from(current.token_hash) : null; const valid = stored && stored.length === candidate.length && timingSafeEqual(stored, candidate); const sessionId = current?.session_id == null ? null : this.#session(rawSession); if (!current || current.claim_id !== claimId || current.actor !== actor || current.generation !== generation || (current.session_id !== null && current.device_id !== device) || current.session_id !== sessionId || !valid) throw new DomainError(409, 'stale_claim', 'claim fence is no longer owned by this machine and Pi session'); return current; }
