...
export class Store {
    ...
    this.#transaction(() => {
      this.#db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS execution_starts (
          ...
          target_actor TEXT NOT NULL REFERENCES actors(id),started_by TEXT NOT NULL REFERENCES actors(id),started_at INTEGER NOT NULL,
          consumed_at INTEGER,revoked_at INTEGER,UNIQUE(ticket_id,generation)
        ) STRICT;
        CREATE UNIQUE INDEX IF NOT EXISTS execution_starts_one_current ON execution_starts(ticket_id) WHERE consumed_at IS NULL AND revoked_at IS NULL;
      `);
  ...
  async listTickets(rawProject, { assigneeType, assigneeId, includeArchived = false } = {}) { const project = this.#project(rawProject); const where = ['t.project=?', 't.deleted_at IS NULL']; const values = [project.key]; if (!includeArchived) where.push('t.archived_at IS NULL'); if (assigneeType === 'none') where.push('t.assignee_type IS NULL'); else if (assigneeType || assigneeId) { if (!['actor', 'role'].includes(assigneeType) || !assigneeId) throw new DomainError(400, 'invalid_assignee_filter', 'assignee_type actor|role and assignee_id are required'); where.push('t.assignee_type=? AND t.assignee_id=?'); values.push(assigneeType, stableId(assigneeId)); } return this.#db.prepare(`SELECT t.*,c.claim_id,c.actor claim_actor,c.generation,c.claimed_at,s.id start_id,s.generation start_generation,s.target_actor start_target_actor,s.started_by,s.started_at FROM tickets t LEFT JOIN claims c ON c.ticket_id=t.id AND c.released_at IS NULL LEFT JOIN execution_starts s ON s.ticket_id=t.id AND s.consumed_at IS NULL AND s.revoked_at IS NULL WHERE ${where.join(' AND ')} ORDER BY t.number`).all(...values).map((r) => this.#publicTicket(r)); }
  async getTicket(id) { return this.#ticket(id); }
  #activeStart(id) { return this.#db.prepare('SELECT * FROM execution_starts WHERE ticket_id=? AND consumed_at IS NULL AND revoked_at IS NULL').get(id) ?? null; }
  #revokeStart(ticket, actor, reason) { const start = this.#activeStart(ticket.id); if (!start) return false; const changed = this.#db.prepare('UPDATE execution_starts SET revoked_at=? WHERE id=? AND consumed_at IS NULL AND revoked_at IS NULL').run(this.#now(), start.id); if (changed.changes) this.#event(ticket.id, ticket.project, 'execution_start_revoked', actor, reason, { generation: start.generation, target_actor: start.target_actor, reason }); return Boolean(changed.changes); }
  async startExecution(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#mutableTicket(id); const actor = this.#human(rawActor); if (ticket.state !== 'open' || ticket.claim) throw new DomainError(409, 'execution_start_not_allowed', 'only an unclaimed open ticket can be started'); if (ticket.assignee?.type !== 'actor') throw new DomainError(409, 'execution_assignment_required', 'execution requires exact actor assignment'); const existing = this.#activeStart(id); if (existing?.target_actor === ticket.assignee.id) return this.#ticket(id); if (existing) this.#revokeStart(ticket, actor.id, 'assignment_fence'); const generation = Number(this.#db.prepare('SELECT COALESCE(MAX(generation),0)+1 generation FROM execution_starts WHERE ticket_id=?').get(id).generation); const now = this.#now(); this.#db.prepare('INSERT INTO execution_starts(ticket_id,generation,target_actor,started_by,started_at) VALUES(?,?,?,?,?)').run(id, generation, ticket.assignee.id, actor.id, now); this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(now, id); this.#event(id, ticket.project, 'execution_started', actor.id, 'Execution explicitly started.', { generation, target_actor: ticket.assignee.id }); return this.#ticket(id); }); }
  async revokeExecutionStart(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#mutableTicket(id); const actor = this.#human(rawActor); this.#revokeStart(ticket, actor.id, 'human_revoked'); this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(this.#now(), id); return this.#ticket(id); }); }
  async archiveTicket(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#human(rawActor); if (ticket.deleted_at !== null) throw new DomainError(409, 'ticket_deleted', 'deleted tickets cannot be archived'); if (ticket.archived_at !== null) return ticket; const now = this.#now(); this.#revokeStart(ticket, actor.id, 'archived'); if (ticket.claim) this.#db.prepare('UPDATE claims SET released_at=? WHERE claim_id=?').run(now, ticket.claim.claim_id); this.#db.prepare('UPDATE tickets SET archived_at=?,updated_at=? WHERE id=?').run(now, now, id); this.#event(id, ticket.project, 'archived', actor.id, 'Archived ticket.'); return this.#ticket(id); }); }
  ...
  async claimNext({ project: rawProject, actor: rawActor }) { return this.#transaction(() => { const actor = this.#actor(rawActor); let query = "SELECT t.*,NULL claim_id,NULL claim_actor,NULL generation,NULL claimed_at,s.id start_id,s.generation start_generation,s.target_actor start_target_actor,s.started_by,s.started_at FROM tickets t JOIN execution_starts s ON s.ticket_id=t.id AND s.consumed_at IS NULL AND s.revoked_at IS NULL WHERE t.state='open' AND t.archived_at IS NULL AND t.deleted_at IS NULL AND t.assignee_type='actor' AND t.assignee_id=? AND s.target_actor=? AND NOT EXISTS(SELECT 1 FROM claims c WHERE c.ticket_id=t.id AND c.released_at IS NULL)"; const values = [actor.id, actor.id]; if (rawProject) { query += ' AND t.project=?'; values.push(this.#project(rawProject).key); } query += ' ORDER BY t.created_at,t.project,t.number LIMIT 1'; const row = this.#db.prepare(query).get(...values); if (!row) return null; const ticket = this.#publicTicket(row); const consumed = this.#db.prepare('UPDATE execution_starts SET consumed_at=? WHERE id=? AND consumed_at IS NULL AND revoked_at IS NULL').run(this.#now(), row.start_id); if (consumed.changes !== 1) throw new DomainError(409, 'execution_start_stale', 'execution start is no longer current'); this.#event(ticket.id, ticket.project, 'execution_start_consumed', actor.id, null, { generation: row.start_generation, target_actor: actor.id }); return this.#claim(ticket, actor); }); }
  async editTicket(id, changes = {}) { return this.#transaction(() => {
    ...
    if (assigned) this.#revokeStart(ticket, actor.id, 'assignment_changed');
  ...
  async setTicketState(id, { state, actor: rawActor }) { return this.#transaction(() => {
    ...
    const now = this.#now();
    if (state !== 'open') this.#revokeStart(ticket, actor.id, 'state_changed');
  ...
  async claim(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (ticket.state !== 'open' || ticket.claim || ticket.archived_at !== null || ticket.deleted_at !== null) throw new DomainError(409, 'ticket_unavailable', 'ticket is not available to claim'); if (!this.#eligible(ticket, actor)) throw new DomainError(409, 'assignee_ineligible', 'actor is not eligible for this assignment'); this.#revokeStart(ticket, actor.id, 'claimed_outside_execution_lane'); return this.#claim(ticket, actor); }); }
