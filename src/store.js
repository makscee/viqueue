...
export class Store {
  ...
  #migrate() {
    this.#transaction(() => {
      this.#db.exec(`
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS execution_authorities_one_current ON execution_authorities(ticket_id) WHERE consumed_at IS NULL AND revoked_at IS NULL;
  ...
  async createTicket({ project: rawProject, title, body = '', assignee = null, assignment = undefined, assigned_to = undefined, actor: rawActor = null }, { trustedAssignment = false } = {}) {
    return this.#transaction(() => { const project = this.#project(rawProject); const creator = rawActor == null ? null : this.#human(rawActor); const number = project.next_number; const id = `${project.key}-${number}`; const now = this.#now(); let target = assignee ?? assignment ?? null; const legacy = cleanOptional(assigned_to, 'assigned_to'); if (!target && legacy) target = { type: 'actor', id: legacy }; if (target) target = this.#target(target, { assignment: true }); this.#db.prepare("INSERT INTO tickets(id,project,number,title,body,state,assigned_to,assignee_type,assignee_id,created_at,updated_at) VALUES(?,?,?,?,?,'open',?,?,?,?,?)").run(id, project.key, number, this.#title(title), cleanOptional(body, 'body') ?? '', legacy, target?.type ?? null, target?.id ?? null, now, now); this.#db.prepare('UPDATE projects SET next_number=next_number+1 WHERE key=?').run(project.key); this.#event(id, project.key, 'ticket_created', creator?.id ?? null, null, { assignee: target }); const ticket = this.#ticket(id); if (trustedAssignment && creator && target) this.#grantExecutionAuthority(ticket, target, creator.id); return this.#ticket(id); });
  }
  async listTickets(rawProject, { assigneeType, assigneeId, includeArchived = false } = {}) { const project = this.#project(rawProject); const where = ['t.project=?', 't.deleted_at IS NULL']; const values = [project.key]; if (!includeArchived) where.push('t.archived_at IS NULL'); if (assigneeType === 'none') where.push('t.assignee_type IS NULL'); else if (assigneeType || assigneeId) { if (!['actor', 'role'].includes(assigneeType) || !assigneeId) throw new DomainError(400, 'invalid_assignee_filter', 'assignee_type actor|role and assignee_id are required'); where.push('t.assignee_type=? AND t.assignee_id=?'); values.push(assigneeType, stableId(assigneeId)); } return this.#db.prepare(`SELECT t.*,c.claim_id,c.actor claim_actor,c.generation,c.claimed_at,x.id execution_authority_id,x.generation execution_authority_generation,x.assignee_type execution_assignee_type,x.assignee_id execution_assignee_id,x.granted_by execution_granted_by,x.granted_at execution_granted_at,(SELECT COUNT(*) FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open') unresolved_blockers FROM tickets t LEFT JOIN claims c ON c.ticket_id=t.id AND c.released_at IS NULL LEFT JOIN execution_authorities x ON x.ticket_id=t.id AND x.consumed_at IS NULL AND x.revoked_at IS NULL WHERE ${where.join(' AND ')} ORDER BY t.number`).all(...values).map((r) => this.#publicTicket(r)); }
  async getTicket(id) { return this.#ticket(id); }
  #activeExecutionAuthority(id) { return this.#db.prepare('SELECT * FROM execution_authorities WHERE ticket_id=? AND consumed_at IS NULL AND revoked_at IS NULL').get(id) ?? null; }
  #revokeExecutionAuthority(ticket, actor, reason) { const authority = this.#activeExecutionAuthority(ticket.id); if (!authority) return false; const changed = this.#db.prepare('UPDATE execution_authorities SET revoked_at=? WHERE id=? AND consumed_at IS NULL AND revoked_at IS NULL').run(this.#now(), authority.id); if (changed.changes) this.#event(ticket.id, ticket.project, 'execution_authority_revoked', actor, reason, { generation: authority.generation, assignee: { type: authority.assignee_type, id: authority.assignee_id }, reason }); return Boolean(changed.changes); }
  #grantExecutionAuthority(ticket, assignee, actor) { const current = this.#activeExecutionAuthority(ticket.id); if (current) this.#revokeExecutionAuthority(ticket, actor, 'trusted_assignment_replaced'); const generation = Number(this.#db.prepare('SELECT COALESCE(MAX(generation),0)+1 generation FROM execution_authorities WHERE ticket_id=?').get(ticket.id).generation); const now = this.#now(); this.#db.prepare('INSERT INTO execution_authorities(ticket_id,generation,assignee_type,assignee_id,granted_by,granted_at) VALUES(?,?,?,?,?,?)').run(ticket.id, generation, assignee.type, assignee.id, actor, now); this.#event(ticket.id, ticket.project, 'execution_authority_granted', actor, 'Trusted assignment authorized execution.', { generation, assignee }); }
  async archiveTicket(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#human(rawActor); if (ticket.deleted_at !== null) throw new DomainError(409, 'ticket_deleted', 'deleted tickets cannot be archived'); if (ticket.archived_at !== null) return ticket; const now = this.#now(); this.#revokeExecutionAuthority(ticket, actor.id, 'archived'); if (ticket.claim) this.#db.prepare('UPDATE claims SET released_at=? WHERE claim_id=?').run(now, ticket.claim.claim_id); this.#db.prepare('UPDATE tickets SET archived_at=?,updated_at=? WHERE id=?').run(now, now, id); this.#event(id, ticket.project, 'archived', actor.id, 'Archived ticket.'); return this.#ticket(id); }); }
  ...
  #next(rawProject, actor = null) { let query = "SELECT t.*,NULL claim_id,NULL claim_actor,NULL generation,NULL claimed_at,NULL execution_authority_id,NULL execution_authority_generation,NULL execution_assignee_type,NULL execution_assignee_id,NULL execution_granted_by,NULL execution_granted_at,(SELECT COUNT(*) FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open') unresolved_blockers FROM tickets t WHERE t.state='open' AND t.archived_at IS NULL AND t.deleted_at IS NULL AND NOT EXISTS(SELECT 1 FROM claims c WHERE c.ticket_id=t.id AND c.released_at IS NULL) AND NOT EXISTS(SELECT 1 FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open')"; const values = []; if (rawProject) { query += ' AND t.project=?'; values.push(this.#project(rawProject).key); } query += ' ORDER BY t.created_at,t.project,t.number'; const rows = this.#db.prepare(query).all(...values).map((r) => this.#publicTicket(r)); return actor ? rows.find((t) => this.#eligible(t, actor)) ?? null : rows[0] ?? null; }
  #claim(ticket, actor) { const generation = Number(this.#db.prepare('SELECT COALESCE(MAX(generation),0)+1 generation FROM claims WHERE ticket_id=?').get(ticket.id).generation); const token = randomBytes(32).toString('base64url'); const claimId = randomUUID(); this.#db.prepare('INSERT INTO claims(claim_id,ticket_id,actor,generation,token_hash,claimed_at) VALUES(?,?,?,?,?,?)').run(claimId, ticket.id, actor.id, generation, hash(token), this.#now()); this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(this.#now(), ticket.id); this.#event(ticket.id, ticket.project, 'claimed', actor.id, null, { machine: actor.machine }); return { ticket: this.#ticket(ticket.id), claim_token: token }; }
  async next({ project: rawProject, actor: actorId } = {}) { return this.#next(rawProject, actorId ? this.#actor(actorId) : null); }
  async claimNext({ project: rawProject, actor: rawActor }) { return this.#transaction(() => { const actor = this.#actor(rawActor); let query = "SELECT t.*,NULL claim_id,NULL claim_actor,NULL generation,NULL claimed_at,x.id execution_authority_id,x.generation execution_authority_generation,x.assignee_type execution_assignee_type,x.assignee_id execution_assignee_id,x.granted_by execution_granted_by,x.granted_at execution_granted_at,(SELECT COUNT(*) FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open') unresolved_blockers FROM tickets t JOIN execution_authorities x ON x.ticket_id=t.id AND x.consumed_at IS NULL AND x.revoked_at IS NULL WHERE t.state='open' AND t.archived_at IS NULL AND t.deleted_at IS NULL AND ((x.assignee_type='actor' AND x.assignee_id=?) OR (x.assignee_type='role' AND EXISTS(SELECT 1 FROM actor_roles ar WHERE ar.actor_id=? AND ar.role_id=x.assignee_id))) AND t.assignee_type=x.assignee_type AND t.assignee_id=x.assignee_id AND NOT EXISTS(SELECT 1 FROM claims c WHERE c.ticket_id=t.id AND c.released_at IS NULL) AND NOT EXISTS(SELECT 1 FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open')"; const values = [actor.id, actor.id]; if (rawProject) { query += ' AND t.project=?'; values.push(this.#project(rawProject).key); } query += ' ORDER BY t.created_at,t.project,t.number LIMIT 1'; const row = this.#db.prepare(query).get(...values); if (!row) return null; const ticket = this.#publicTicket(row); const consumed = this.#db.prepare('UPDATE execution_authorities SET consumed_at=? WHERE id=? AND consumed_at IS NULL AND revoked_at IS NULL').run(this.#now(), row.execution_authority_id); if (consumed.changes !== 1) throw new DomainError(409, 'execution_authority_stale', 'assignment execution authority is no longer current'); this.#event(ticket.id, ticket.project, 'execution_authority_consumed', actor.id, null, { generation: row.execution_authority_generation, assignee: ticket.assignee }); return this.#claim(ticket, actor); }); }
  async editTicket(id, changes = {}, { trustedAssignment = false } = {}) { return this.#transaction(() => {
    ...
    let edited = false; let assigned = false; let moved = false; const assignmentTouched = 'assignee' in changes || 'assignment' in changes || 'assigned_to' in changes;
    ...
    if (!edited && !assigned && !moved && !(trustedAssignment && assignmentTouched && assignee && !ticket.execution_authority)) return ticket;
    if (assignmentTouched) this.#revokeExecutionAuthority(ticket, actor.id, trustedAssignment ? 'trusted_assignment_replaced' : 'untrusted_assignment');
    ...
    if (trustedAssignment && assignmentTouched && assignee) this.#grantExecutionAuthority(this.#ticket(id), assignee, actor.id);
  ...
  async setTicketState(id, { state, actor: rawActor }) { return this.#transaction(() => {
    ...
    const now = this.#now();
    if (state !== 'open') this.#revokeExecutionAuthority(ticket, actor.id, 'state_changed');
  ...
  async claim(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (ticket.state !== 'open' || ticket.claim || ticket.archived_at !== null || ticket.deleted_at !== null) throw new DomainError(409, 'ticket_unavailable', 'ticket is not available to claim'); if (!this.#eligible(ticket, actor)) throw new DomainError(409, 'assignee_ineligible', 'actor is not eligible for this assignment'); this.#revokeExecutionAuthority(ticket, actor.id, 'claimed_outside_execution_lane'); return this.#claim(ticket, actor); }); }
