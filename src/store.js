...
export class Store {
  ...
  async createLocalBrowserPairing({ device_name: deviceName, ttl_ms: ttlMs = 900000 } = {}) { return this.#transaction(() => {
    ...
    const issued = this.#createPairingCode(this.#adminDevice(creator.id), { intended_kind: 'coordinator', actor_id: this.#device(creator.id).actor_id, device_id: deviceId, device_name: name, ttl_ms: ttlMs });
  ...
  async pairDevice({ code, id, name }, { requiredKind = null, requireBoundIdentity = false } = {}) { return this.#transaction(() => {
    ...
    if (requireBoundIdentity && (!row.device_id || !row.device_name || !row.actor_id)) throw new DomainError(409, 'pairing_device_binding_required', 'browser pairing requires a server-bound identity');
  ...
  async createTicket(input = {}) {
    if ('projects' in input || 'assignee' in input || 'assigned_to' in input || 'body' in input) throw new DomainError(400, 'invalid_ticket_fields', 'ticket create accepts only project, title, description, assignment, and worker_actor_id');
    const { project: rawProject, title, description = '', assignment = 'Unassigned', worker_actor_id: workerActorId = null, actor: rawActor = null } = input;
    return this.#transaction(() => { const project = this.#project(rawProject); const creator = rawActor == null ? null : this.#adminDevice(rawActor); if (!['Unassigned','Human','Agent'].includes(assignment)) throw new DomainError(400, 'invalid_assignment', 'assignment must be Unassigned, Human, or Agent'); let worker = null; if (workerActorId != null) { worker = this.#actor(workerActorId); if (assignment !== 'Agent' || worker.kind !== 'agent' || !this.#db.prepare("SELECT 1 FROM devices WHERE actor_id=? AND kind='worker' AND status='active'").get(worker.id)) throw new DomainError(400, 'invalid_worker_assignment', 'worker_actor_id must identify an active paired worker'); } const number = project.next_number; const id = `${project.key}-${number}`; const now = this.#now(); this.#db.prepare("INSERT INTO tickets(id,project,number,title,body,state,assigned_to,assignee_type,assignee_id,assignment,created_at,updated_at) VALUES(?,?,?,?,?,'open',NULL,?,?,?,?,?)").run(id, project.key, number, this.#title(title), cleanOptional(description, 'description') ?? '', worker ? 'actor' : null, worker?.id ?? null, assignment, now, now); this.#db.prepare('UPDATE projects SET next_number=next_number+1 WHERE key=?').run(project.key); this.#db.prepare('INSERT INTO ticket_projects(ticket_id,project_key) VALUES(?,?)').run(id, project.key); this.#event(id, project.key, 'ticket_created', creator?.id ?? null, null, { assignment, worker_actor_id: worker?.id ?? null }); return this.#ticket(id); });
  }
