...
export class Store {
  ...
  #claim(ticket, device, sessionId, sessionCapability) { if (this.#db.prepare('SELECT 1 FROM claims WHERE device_id=? AND released_at IS NULL').get(device.id)) throw new DomainError(409, 'machine_already_claimed', 'this machine already has an active claim'); const generation = Number(this.#db.prepare('SELECT COALESCE(MAX(generation),0)+1 generation FROM claims WHERE ticket_id=?').get(ticket.id).generation); const token = randomBytes(32).toString('base64url'); const claimId = randomUUID(); this.#db.prepare('INSERT INTO claims(claim_id,ticket_id,actor,generation,token_hash,claimed_at,device_id,session_id) VALUES(?,?,?,?,?,?,?,?)').run(claimId, ticket.id, device.actor_id, generation, hash(token), this.#now(), device.id, sessionId); this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(this.#now(), ticket.id); this.#event(ticket.id, ticket.project, 'claimed', device.actor_id, null, { session_bound: sessionId !== null, machine: device.name }, device.id); return { ticket: this.#ticket(ticket.id), claim_token: token, ...(sessionCapability ? { session_capability: sessionCapability } : {}) }; }
  async next({ device: deviceId, actor: legacyId } = {}) { const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); return this.#next(device); }
  async claimNext({ device: deviceId, actor: legacyId, session_capability: capability }) { return this.#transaction(() => { const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const opened = capability ? null : this.#insertWorkerSession(device); capability ??= opened.session_capability; const sessionId = this.#sessionCapability(capability, device.id).id; const ticket = this.#next(device); return ticket ? this.#claim(ticket, device, sessionId, opened?.session_capability) : null; }); }
