...
import { normalizeReviewBundle } from './review-bundle.js';
...
export class Store {
    ...
    this.#transaction(() => {
      this.#db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS worker_presence (
          device_id TEXT PRIMARY KEY REFERENCES devices(id),mode TEXT NOT NULL CHECK(mode IN ('waiting','working','stopped')),
          ticket_id TEXT REFERENCES tickets(id),worker_heartbeat_at INTEGER NOT NULL,ticket_heartbeat_at INTEGER
        ) STRICT;
        CREATE TABLE IF NOT EXISTS worker_mutation_receipts (
          request_id TEXT PRIMARY KEY,ticket_id TEXT NOT NULL,claim_id TEXT NOT NULL,generation INTEGER NOT NULL,
          device_id TEXT NOT NULL,kind TEXT NOT NULL,result_json TEXT NOT NULL,created_at INTEGER NOT NULL
        ) STRICT;
        CREATE INDEX IF NOT EXISTS worker_mutation_receipts_created ON worker_mutation_receipts(created_at);
        ...
        CREATE TABLE IF NOT EXISTS project_review_policies (
          project_key TEXT PRIMARY KEY REFERENCES projects(key),proof_class TEXT NOT NULL CHECK(proof_class IN ('human-readable','visual'))
        ) STRICT;
      ...
      if (!claimColumns.includes('lease_expires_at')) this.#db.exec('ALTER TABLE claims ADD COLUMN lease_expires_at INTEGER');
      this.#db.exec('CREATE INDEX IF NOT EXISTS claims_worker_lease ON claims(lease_expires_at) WHERE released_at IS NULL AND lease_expires_at IS NOT NULL');
      this.#db.exec('DROP INDEX IF EXISTS claims_one_current_device');
      this.#db.exec('CREATE UNIQUE INDEX IF NOT EXISTS claims_one_current_session ON claims(session_id) WHERE released_at IS NULL AND session_id IS NOT NULL');
      const ticketColumns = this.#columns('tickets');
      ...
      if (!ticketColumns.includes('deployment_status')) this.#db.exec("ALTER TABLE tickets ADD COLUMN deployment_status TEXT NOT NULL DEFAULT 'not-released' CHECK(deployment_status IN ('not-released','released','production-verified'))");
      if (!ticketColumns.includes('deployment_reference')) this.#db.exec('ALTER TABLE tickets ADD COLUMN deployment_reference TEXT');
      if (!ticketColumns.includes('deployment_updated_at')) this.#db.exec('ALTER TABLE tickets ADD COLUMN deployment_updated_at INTEGER');
  ...
  async createProject(raw) { return this.#transaction(() => { const object=raw&&typeof raw==='object',key=String(object?raw.key:raw??'').toUpperCase(),proof=object?(raw.review_proof_class??'human-readable'):'human-readable'; if (!/^[A-Z][A-Z0-9]{1,9}$/.test(key)) throw new DomainError(400, 'invalid_project_key', 'project key must be 2-10 uppercase letters or digits'); if(!['human-readable','visual'].includes(proof))throw new DomainError(400,'invalid_review_proof_class','review_proof_class must be human-readable or visual'); try { this.#db.prepare('INSERT INTO projects(key,next_number,created_at) VALUES(?,1,?)').run(key, this.#now()); this.#db.prepare('INSERT INTO project_review_policies(project_key,proof_class) VALUES(?,?)').run(key,proof); } catch (error) { if (error.code?.startsWith('ERR_SQLITE_CONSTRAINT')) throw new DomainError(409, 'project_exists', `project ${key} already exists`); throw error; } const project=this.#project(key);return object?{...project,review_proof_class:proof}:project; }); }
  async listProjects() { return this.#db.prepare("SELECT p.key,p.next_number,p.created_at,COALESCE(r.proof_class,'human-readable') review_proof_class FROM projects p LEFT JOIN project_review_policies r ON r.project_key=p.key ORDER BY p.key").all(); }
  async cleanSlateProjectsAndTickets() { return this.#transaction(() => {
    ...
    this.#db.exec(`
      ...
      DELETE FROM project_review_policies;
  ...
  async closeWorkerSession(deviceId, capability) { return this.#transaction(() => { const session = this.#sessionCapability(capability, this.#device(deviceId, { kind: 'worker' }).id), now = this.#now(); const claims = this.#db.prepare('SELECT claim_id,ticket_id FROM claims WHERE session_id=? AND released_at IS NULL').all(session.id); this.#db.prepare('UPDATE claims SET released_at=? WHERE session_id=? AND released_at IS NULL').run(now, session.id); for (const claim of claims) this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(now, claim.ticket_id); this.#db.prepare('UPDATE worker_sessions SET revoked_at=? WHERE id=? AND revoked_at IS NULL').run(now, session.id); return { session_id: session.id, revoked: true }; }); }
  async heartbeatWorker(deviceId, { mode = 'waiting', ticket_id: ticketId = null, claim_id: claimId = null, generation = null, claim_token: token = null, session_capability: capability } = {}) { return this.#transaction(() => { const device=this.#device(deviceId,{kind:'worker'}), now=this.#now(); if (!['waiting','working','stopped'].includes(mode)) throw new DomainError(400,'invalid_worker_mode','worker mode is invalid'); let ticketHeartbeat=null; if(mode==='working'){if(typeof ticketId!=='string')throw new DomainError(400,'invalid_ticket','working heartbeat requires ticket'); const claim=this.#authority(ticketId,{claim_id:claimId,actor:device.actor_id,device:device.id,generation,claim_token:token,session_capability:capability});this.#db.prepare('UPDATE claims SET lease_expires_at=? WHERE claim_id=?').run(now+90000,claim.claim_id); ticketHeartbeat=now;} this.#db.prepare("INSERT INTO worker_presence(device_id,mode,ticket_id,worker_heartbeat_at,ticket_heartbeat_at) VALUES(?,?,?,?,?) ON CONFLICT(device_id) DO UPDATE SET mode=excluded.mode,ticket_id=excluded.ticket_id,worker_heartbeat_at=excluded.worker_heartbeat_at,ticket_heartbeat_at=excluded.ticket_heartbeat_at").run(device.id,mode,mode==='working'?ticketId:null,now,ticketHeartbeat); return { worker_heartbeat_at:now,ticket_heartbeat_at:ticketHeartbeat,lease_expires_at:ticketHeartbeat?now+90000:null }; }); }
  async listWorkerPresence(staleAfterMs = 90000) {
    const cutoff=this.#now()-staleAfterMs;
    const rows=this.#db.prepare("SELECT d.id,d.name,p.mode,p.ticket_id,p.worker_heartbeat_at,p.ticket_heartbeat_at FROM devices d LEFT JOIN worker_presence p ON p.device_id=d.id WHERE d.kind='worker' AND d.status='active' ORDER BY d.name,d.id").all();
    return rows.map(row=>({
      id:row.id,name:row.name,
      state:!row.worker_heartbeat_at||row.worker_heartbeat_at<cutoff?'stale/offline':row.mode==='working'?'working':'waiting',
      ticket_id:row.mode==='working'?row.ticket_id:null,
      worker_heartbeat_at:row.worker_heartbeat_at??null,
      ticket_heartbeat_at:row.mode==='working'?(row.ticket_heartbeat_at??null):null
    }));
  }
  #next(device, rawProject = null) { const project = rawProject == null ? null : this.#project(rawProject).key; const row = this.#db.prepare(`SELECT t.id,t.project,t.number,t.title,t.body,t.state,t.assigned_to,t.assignment,t.created_at,t.updated_at,t.board_state,CAST(t.board_order AS TEXT) board_order,t.assignee_type,t.assignee_id,t.archived_at,t.deleted_at,t.deployment_status,t.deployment_reference,t.deployment_updated_at,NULL claim_id,NULL claim_actor,NULL claim_device_id,NULL claim_session_id,NULL claim_machine,NULL generation,NULL claimed_at,(SELECT COUNT(*) FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open') unresolved_blockers,0 open_questions FROM tickets t WHERE t.state='open' AND t.board_state='Open' AND t.assignment='Agent' AND t.archived_at IS NULL AND t.deleted_at IS NULL AND (? IS NULL OR t.project=?) AND (t.assignee_type IS NULL OR (t.assignee_type='actor' AND t.assignee_id=?)) AND NOT EXISTS(SELECT 1 FROM claims c WHERE c.ticket_id=t.id AND c.released_at IS NULL) AND NOT EXISTS(SELECT 1 FROM ticket_blocks b WHERE b.ticket_id=t.id AND b.status='open') ORDER BY t.board_order DESC,t.id LIMIT 1`).get(project, project, device.actor_id); const ticket = row ? this.#publicTicket(row) : null; return ticket && this.#claimable(ticket, device) ? ticket : null; }
  #expireWorkerLeases() { const now=this.#now(), expired=this.#db.prepare('SELECT c.claim_id,c.ticket_id,c.actor,c.device_id,t.project FROM claims c JOIN tickets t ON t.id=c.ticket_id WHERE c.released_at IS NULL AND c.device_id IS NOT NULL AND c.lease_expires_at IS NOT NULL AND c.lease_expires_at<=?').all(now); for(const claim of expired){this.#db.prepare('UPDATE claims SET released_at=? WHERE claim_id=? AND released_at IS NULL').run(now,claim.claim_id);this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(now,claim.ticket_id);this.#event(claim.ticket_id,claim.project,'worker_lease_expired',claim.actor,'Worker lease expired; ticket recovered.',{claim_id:claim.claim_id},claim.device_id);} }
  #claim(ticket, device, sessionId, leased=false) { if (this.#db.prepare('SELECT 1 FROM claims WHERE session_id=? AND released_at IS NULL').get(sessionId)) throw new DomainError(409, 'session_already_claimed', 'this Pi session already has an active claim'); const generation = Number(this.#db.prepare('SELECT COALESCE(MAX(generation),0)+1 generation FROM claims WHERE ticket_id=?').get(ticket.id).generation), now=this.#now(), expiry=leased?now+90000:null; const token = randomBytes(32).toString('base64url'); const claimId = randomUUID(); this.#db.prepare('INSERT INTO claims(claim_id,ticket_id,actor,generation,token_hash,claimed_at,device_id,session_id,lease_expires_at) VALUES(?,?,?,?,?,?,?,?,?)').run(claimId, ticket.id, device.actor_id, generation, hash(token), now, device.id, sessionId,expiry); this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(now, ticket.id); this.#event(ticket.id, ticket.project, 'claimed', device.actor_id, null, { session_bound: sessionId !== null, machine: device.name,...(expiry?{lease_expires_at:expiry}:{}) }, device.id); return { ticket: this.#ticket(ticket.id), claim_token: token }; }
  async next(options = {}) { const { device: deviceId, actor: legacyId, project } = options; const projectKey = Object.hasOwn(options, 'project') ? this.#requiredProject(project) : null; const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); return this.#next(device, projectKey); }
  async claimNext(options = {}) { const { device: deviceId, actor: legacyId, session_capability: capability, project, worker_lease: leased=false } = options; const projectKey = Object.hasOwn(options, 'project') ? this.#requiredProject(project) : null; return this.#transaction(() => { if(leased!==true&&leased!==false)throw new DomainError(400,'invalid_worker_lease','worker_lease must be boolean'); if(leased)this.#expireWorkerLeases(); const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const sessionId = this.#sessionCapability(capability, device.id).id; const ticket = this.#next(device, projectKey); return ticket ? this.#claim(ticket, device, sessionId,leased) : null; }); }
  ...
  async setTicketState(id, { state, actor: rawActor, proof_acknowledged=false }) { return this.#transaction(() => {
    ...
    if (ticket.state === 'Waiting' && state !== 'Waiting') {
      if(state==='Done'){const submitted=this.#db.prepare("SELECT metadata FROM events WHERE ticket_id=? AND type='submitted' ORDER BY id DESC LIMIT 1").get(id),bundle=parseMetadata(submitted?.metadata)?.review_bundle??null,policy=this.#db.prepare('SELECT proof_class FROM project_review_policies WHERE project_key=?').get(ticket.project)?.proof_class??'human-readable',visual=Boolean(bundle?.preview_url||(bundle?.screenshots?.length));if(((bundle?.ui_change===true)||policy==='visual')&&!visual&&proof_acknowledged!==true)throw new DomainError(409,'proof_acknowledgement_required','visual proof is absent; use the approval and explicitly acknowledge that absence before Human Accept');}
  ...
  async claim(id, { device: deviceId, actor: legacyId, session_capability: capability } = {}) { return this.#transaction(() => { this.#expireWorkerLeases(); const ticket = this.#ticket(id); const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const sessionId = this.#sessionCapability(capability, device.id).id; if (!this.#claimable(ticket, device)) throw new DomainError(409, 'ticket_ineligible', 'ticket is not eligible for this paired worker'); return this.#claim(ticket, device, sessionId); }); }
  #authority(id, { claim_id: claimId, actor, device, session_capability: capability, generation, claim_token: token }) { let current = this.#db.prepare('SELECT * FROM claims WHERE ticket_id=? AND released_at IS NULL').get(id); if(current?.lease_expires_at!=null&&current.lease_expires_at<=this.#now()){this.#expireWorkerLeases();current=null} let session = null; try { session = this.#sessionCapability(capability, device); } catch {} const candidate = typeof token === 'string' ? hash(token) : Buffer.alloc(0); const stored = current?.token_hash ? Buffer.from(current.token_hash) : null; const valid = stored && stored.length === candidate.length && timingSafeEqual(stored, candidate); if (!current || current.claim_id !== claimId || current.actor !== actor || current.generation !== generation || current.device_id !== device || !session || current.session_id !== session.id || !valid) throw new DomainError(409, 'stale_claim', 'claim fence is no longer owned by this machine and live Pi session'); return current; }
  async verify(id, identity) { this.#row(id); this.#authority(id, identity); return this.#ticket(id); }
  #receipt(id,input,kind,work){const requestId=cleanOptional(input.request_id,'request_id');if(!requestId)return work();const prior=this.#db.prepare('SELECT * FROM worker_mutation_receipts WHERE request_id=?').get(requestId);if(prior){let session=null;try{session=this.#sessionCapability(input.session_capability,input.device)}catch{}const tokenHash=typeof input.claim_token==='string'?hash(input.claim_token):Buffer.alloc(0),stored=prior.claim_token_hash?Buffer.from(prior.claim_token_hash):null,validToken=stored&&stored.length===tokenHash.length&&timingSafeEqual(stored,tokenHash);if(prior.ticket_id!==id||prior.claim_id!==input.claim_id||prior.generation!==input.generation||prior.device_id!==input.device||prior.session_id!==session?.id||!validToken||prior.kind!==kind)throw new DomainError(409,'stale_claim','mutation retry is not from its exact original fenced session');return JSON.parse(prior.result_json)}const result=work(),claim=this.#db.prepare('SELECT session_id,token_hash FROM claims WHERE claim_id=?').get(input.claim_id);this.#db.prepare('INSERT INTO worker_mutation_receipts(request_id,ticket_id,claim_id,generation,device_id,kind,result_json,created_at,session_id,claim_token_hash) VALUES(?,?,?,?,?,?,?,?,?,?)').run(requestId,id,input.claim_id,input.generation,input.device,kind,JSON.stringify(result),this.#now(),claim.session_id,claim.token_hash);this.#db.prepare('DELETE FROM worker_mutation_receipts WHERE created_at<?').run(this.#now()-7*86400000);return result;}
