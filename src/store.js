...
export class Store {
  ...
  #mutableTicket(id) { const ticket = this.#ticket(id); if (ticket.deleted_at) throw new DomainError(409, 'ticket_deleted', 'deleted tickets are immutable tombstones'); return ticket; }
  ...
  async archiveTicket(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (actor.kind !== 'human') throw new DomainError(403, 'human_required', 'a human actor is required'); if (ticket.deleted_at) throw new DomainError(409, 'ticket_deleted', 'deleted tickets cannot be archived'); if (ticket.archived_at) return ticket; const now = this.#now(); if (ticket.claim) this.#db.prepare('UPDATE claims SET released_at=? WHERE claim_id=?').run(now, ticket.claim.claim_id); this.#db.prepare('UPDATE tickets SET archived_at=?,updated_at=? WHERE id=?').run(now, now, id); this.#event(id, ticket.project, 'archived', actor.id, 'Archived ticket.'); return this.#ticket(id); }); }
  async restoreTicket(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (actor.kind !== 'human') throw new DomainError(403, 'human_required', 'a human actor is required'); if (ticket.deleted_at) throw new DomainError(409, 'ticket_deleted', 'deleted tickets cannot be restored'); if (!ticket.archived_at) return ticket; this.#db.prepare('UPDATE tickets SET archived_at=NULL,updated_at=? WHERE id=?').run(this.#now(), id); this.#event(id, ticket.project, 'restored', actor.id, 'Restored ticket from archive.'); return this.#ticket(id); }); }
  async deleteTicket(id, { actor: rawActor, confirmed = false }) { return this.#transaction(() => { if (!confirmed) throw new DomainError(409, 'delete_confirmation_required', 'explicit delete confirmation is required'); const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (actor.kind !== 'human') throw new DomainError(403, 'human_required', 'a human actor is required'); if (ticket.deleted_at) return ticket; const now = this.#now(); if (ticket.claim) this.#db.prepare('UPDATE claims SET released_at=? WHERE claim_id=?').run(now, ticket.claim.claim_id); this.#db.prepare('UPDATE tickets SET deleted_at=?,updated_at=? WHERE id=?').run(now, now, id); this.#event(id, ticket.project, 'deleted', actor.id, 'Deleted ticket (history retained as a tombstone).'); return this.#ticket(id); }); }
  ...
  async claim(id, { actor: rawActor }) { return this.#transaction(() => { const ticket = this.#ticket(id); const actor = this.#actor(rawActor); if (ticket.state !== 'open' || ticket.claim || ticket.archived_at || ticket.deleted_at) throw new DomainError(409, 'ticket_unavailable', 'ticket is not available to claim'); if (!this.#eligible(ticket, actor)) throw new DomainError(409, 'assignee_ineligible', 'actor is not eligible for this assignment'); const generation = Number(this.#db.prepare('SELECT COALESCE(MAX(generation),0)+1 generation FROM claims WHERE ticket_id=?').get(id).generation); const token = randomBytes(32).toString('base64url'); const claimId = randomUUID(); this.#db.prepare('INSERT INTO claims(claim_id,ticket_id,actor,generation,token_hash,claimed_at) VALUES(?,?,?,?,?,?)').run(claimId, id, actor.id, generation, hash(token), this.#now()); this.#db.prepare('UPDATE tickets SET updated_at=? WHERE id=?').run(this.#now(), id); this.#event(id, ticket.project, 'claimed', actor.id, null, { machine: actor.machine }); return { ticket: this.#ticket(id), claim_token: token }; }); }
