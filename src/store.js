...
export class Store {
  ...
  async claimNext({ device: deviceId, actor: legacyId, session_capability: capability, project = null } = {}) { return this.#transaction(() => { const device = this.#device(deviceId ?? legacyId, { kind: 'worker' }); const sessionId = this.#sessionCapability(capability, device.id).id; const ticket = this.#next(device, project); return ticket ? this.#claim(ticket, device, sessionId) : null; }); }
