...
async function json(request) {
  let raw = '';
  for await (const chunk of request) { raw += chunk; if (raw.length > 1_000_000) throw new DomainError(413, 'body_too_large', 'request body exceeds 1MB'); }
  try { return raw ? JSON.parse(raw) : {}; } catch { throw new DomainError(400, 'invalid_json', 'request body must be valid JSON'); }
}

const authorize = (request, token) => {
  if (!token || request.headers.authorization !== `Bearer ${token}`) throw new DomainError(403, 'operator_forbidden', 'valid local operator authorization is required');
};

export async function createApp({ storage, operatorToken, now } = {}) {
  const store = new Store(storage ?? './viqueue.sqlite', { now });
  await store.init();
  const localToken = operatorToken;
  const server = http.createServer(async (request, response) => {
    try {
      const url = new URL(request.url, 'http://localhost');
      let match;
      if (request.method === 'GET' && assets[url.pathname]) {
        const [file, contentType] = assets[url.pathname]; response.statusCode = 200; response.setHeader('content-type', contentType);
      ...
      if (request.method === 'GET' && url.pathname === '/v1/tickets/next') {
        const ticket = await store.next({ project: url.searchParams.get('project') }); return send(response, ticket ? 200 : 204, ticket ? { ticket } : undefined);
      }
      ...
      if ((match = url.pathname.match(/^\/v1\/projects\/([^/]+)\/tickets$/)) && request.method === 'GET') return send(response, 200, { tickets: await store.listTickets(decodeURIComponent(match[1])) });
      ...
      if ((match = url.pathname.match(/^\/v1\/tickets\/([^/]+)\/(claim|verify|release|takeover|events|submit|accept|reopen)$/)) && request.method === 'POST') {
        ...
        if (['takeover', 'accept', 'reopen'].includes(action)) authorize(request, localToken);
        ...
        if (action === 'submit') return send(response, 200, { ticket: await store.submit(id, body) });
      ...
      console.error(error); return send(response, 500, { error: { code: 'internal_error', message: 'internal server error' } });
    }
  });
...
export async function runServer({ storage, host = '127.0.0.1', port = 7373, operatorToken } = {}) {
  const server = await createApp({ storage, operatorToken });
  server.listen(port, host, () => process.stdout.write(`${JSON.stringify({ event: 'listening', url: `http://${host}:${port}`, storage })}\n`));
  return server;
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const options = Object.fromEntries(process.argv.slice(2).map((arg) => { const [key, ...rest] = arg.replace(/^--/, '').split('='); return [key.replaceAll('-', '_'), rest.join('=')]; }));
  await runServer({ storage: options.storage ?? process.env.VIQ_STORAGE ?? './viqueue.sqlite', host: options.host ?? process.env.VIQ_HOST ?? '127.0.0.1', port: Number(options.port ?? process.env.VIQ_PORT ?? 7373), operatorToken: options.operator_token ?? process.env.VIQ_OPERATOR_TOKEN });
}
