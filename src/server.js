...
export async function createApp({ storage, operatorToken, now } = {}) {
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if ((match = url.pathname.match(/^\/v1\/tickets\/([^/]+)\/(start-execution|revoke-start)$/)) && request.method === 'POST') { const ticket = match[2] === 'start-execution' ? await store.startExecution(decodeURIComponent(match[1]), await json(request)) : await store.revokeExecutionStart(decodeURIComponent(match[1]), await json(request)); return send(response, 200, { ticket }); }
