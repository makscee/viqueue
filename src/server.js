...
const trustedAssignment = (request, ingressToken, operatorToken) => Boolean((ingressToken && request.headers['x-viq-internal-auth'] === `Bearer ${ingressToken}`) || (operatorToken && request.headers.authorization === `Bearer ${operatorToken}`));

export async function createApp({ storage, operatorToken, ingressToken, now } = {}) {
  const store = new Store(storage ?? './viqueue.sqlite', { now }); await store.init(); const localToken = operatorToken; const localIngressToken = ingressToken;
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/v1/tickets') return send(response, 201, { ticket: await store.createTicket(await json(request), { trustedAssignment: trustedAssignment(request, localIngressToken, localToken) }) });
      ...
      if ((match = url.pathname.match(/^\/v1\/tickets\/([^/]+)$/)) && request.method === 'PATCH') return send(response, 200, { ticket: await store.editTicket(decodeURIComponent(match[1]), await json(request), { trustedAssignment: trustedAssignment(request, localIngressToken, localToken) }) });
...
export async function runServer({ storage, host = '127.0.0.1', port = 7373, operatorToken, ingressToken } = {}) { const server = await createApp({ storage, operatorToken, ingressToken }); server.listen(port, host, () => process.stdout.write(`${JSON.stringify({ event: 'listening', url: `http://${host}:${port}`, storage })}\n`)); return server; }
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) { const options = Object.fromEntries(process.argv.slice(2).map((arg) => { const [key, ...rest] = arg.replace(/^--/, '').split('='); return [key.replaceAll('-', '_'), rest.join('=')]; })); await runServer({ storage: options.storage ?? process.env.VIQ_STORAGE ?? './viqueue.sqlite', host: options.host ?? process.env.VIQ_HOST ?? '127.0.0.1', port: Number(options.port ?? process.env.VIQ_PORT ?? 7373), operatorToken: options.operator_token ?? process.env.VIQ_OPERATOR_TOKEN, ingressToken: options.ingress_token ?? process.env.VIQ_INGRESS_TOKEN }); }
