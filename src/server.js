...
const sessionClaim = (body, device) => { if (typeof body?.session_id !== 'string') throw new DomainError(400, 'invalid_session_id', 'an opaque active Pi session id is required'); return { device: device.id, session_id: body.session_id }; };
...
export async function createApp({ storage, now } = {}) {
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/v1/tickets/claim-next') { requireKind(device, 'worker'); const claim = await store.claimNext(sessionClaim(await json(request), device)); return send(response, claim ? 200 : 204, claim ?? undefined); }
      ...
      if ((match = url.pathname.match(/^\/v1\/tickets\/([^/]+)\/(claim|verify|release|events|block|submit|accept|reopen)$/)) && request.method === 'POST') {
        ...
        if (action === 'claim') { requireKind(device, 'worker'); return send(response, 200, await store.claim(id, sessionClaim(body, device))); }
