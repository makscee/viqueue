...
export async function createApp({ storage, operatorToken, now } = {}) {
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if ((match = url.pathname.match(/^\/v1\/projects\/([^/]+)\/tickets$/)) && request.method === 'GET') return send(response, 200, { tickets: await store.listTickets(decodeURIComponent(match[1]), { assigneeType: url.searchParams.get('assignee_type') ?? undefined, assigneeId: url.searchParams.get('assignee_id') ?? undefined }) });
