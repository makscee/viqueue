...
const MAX_TITLE=200,MAX_DESCRIPTION=20000,MAX_NOTE=8000,MAX_METADATA=2048;
...
// Upstream imposes no ceiling on metadata either, so this one has to hold it.
const boundedMetadata=(value)=>objectBody(value)&&Buffer.byteLength(JSON.stringify(value))<=MAX_METADATA;
const noteBody=(value)=>objectBody(value)&&onlyKeys(value,new Set(['message','metadata']))&&filledText(value.message,MAX_NOTE)&&(!('metadata' in value)||value.metadata===null||boundedMetadata(value.metadata));
...
export async function createPhoneGateway({authDb,origin,upstream,upstreamAddressPolicy,upstreamAuthorization,cert,key,tlsTerminated=false,testMode=false,testHooks,now}={}){
 ...
 // A refusal names its class: a boundary the caller crossed is 403, an upstream that could not answer is 502.
 const refuse=(status,reason)=>Object.assign(new Error(),{status,reason});
 const answered=(error)=>Number.isInteger(error?.upstreamStatus)&&error.upstreamStatus<500;
 const askUpstream=async(targetPath,authorization,crossed,unavailable)=>{try{return await upstreamJson(targetPath,authorization)}catch(error){throw answered(error)?refuse(403,crossed):refuse(502,unavailable)}};
 const delegatedBody=(write,raw)=>{let data;try{data=parse(raw)}catch(error){throw Object.assign(error,{reason:'body_unparsable'})}if(!write.valid(data))throw refuse(403,'schema_refused');return data};
 const delegatedObject=async(write)=>{if(write.ticket===null)return;const lookup=await askUpstream(`/v1/tickets/${encodeURIComponent(write.ticket)}`,undefined,'object_absent','object_lookup_failed');if(lookup?.ticket?.id!==write.ticket||lookup.ticket.project!==VC_PROJECT)throw refuse(403,'object_outside_project')};
 // Schema first, then the object: a body the broker has already refused never becomes an upstream request.
 const delegatedCheck=async(write,raw)=>{const data=delegatedBody(write,raw);await delegatedObject(write);return data};
 // Anonymous probing gets its own, far tighter bucket than sourceLimited: only an identity lookup that
 ...
 // real refusals out of the fifty rows status() shows (phone-auth-store.js:34).
 const probeCapped=(req)=>{const bucket=probeBucket(req);if(bucket.n<PROBE_LIMIT)return null;const first=!bucket.noted;bucket.noted=true;return Object.assign(refuse(429,'rate_limited'),first?{}:{silent:true})};
 const handler=async(req,res)=>{secure(res);try{const rawTarget=requestTarget(req),pathOnly=rawTarget.split('?',1)[0];
  ...
  const raw=await body(req,1048576);let vcWrite=null,vcTicketWrite=null,payload=null;if(isApi){if(!upstreamAuthorization)throw new Error();if(sourceLimited(req))throw Object.assign(new Error(),{status:429});const writeRoute=delegatedWrite(req.method,rawTarget,pathOnly),d=req.headers['x-viq-device'],id=req.headers['x-viq-challenge'],sig=req.headers['x-viq-signature'];const record=(data,actorId,deviceId,authMode)=>{if(writeRoute.operation==='state'){vcWrite={authMode,ticket:writeRoute.ticket,state:data.state,device:deviceId};return}if(writeRoute.operation==='note')payload=Buffer.from(JSON.stringify(attributedNote(data,actorId,deviceId)));vcTicketWrite={authMode,operation:writeRoute.operation,actor:actorId,device:deviceId,detail:writeRoute.ticket===null?{}:{ticket_id:writeRoute.ticket}}};const bearerMode=Boolean(writeRoute&&d===undefined&&id===undefined&&sig===undefined);if(bearerMode){let deniedDevice=null;try{const authorization=req.headers.authorization;if(typeof authorization!=='string'||authorization.length>519||!/^Bearer [\x21-\x7e]{1,512}$/.test(authorization))throw refuse(403,'credential_malformed');const data=delegatedBody(writeRoute,raw);const capped=probeCapped(req);if(capped)throw capped;const identity=await askUpstream('/v1/devices/me',authorization.slice(7),'identity_unresolved','identity_lookup_failed');if(identity?.actor?.id!=='artem'||identity.actor.active!==true||identity.actor.admin!==false||identity?.device?.id!=='artems-macbook-pro'||identity.device.kind!=='coordinator'||identity.device.status!=='active'||identity.device.admin!==false)throw refuse(403,'identity_mismatch');deniedDevice=identity.device.id;await delegatedObject(writeRoute);record(data,identity.actor.id,identity.device.id,'bearer-delegation')}catch(error){const reason=error?.reason??'refused';if(IDENTITY_FAILURES.has(reason))spendProbe(req);if(!error?.silent)store.audit('vc_delegation_denied',deniedDevice,JSON.stringify({auth_mode:'bearer-delegation',method:req.method,target:rawTarget,reason}));throw error}
  }else{if(typeof d==='string'&&deviceLimited(d))throw Object.assign(new Error(),{status:429});if([d,id,sig].some(x=>typeof x!=='string'||x.length>512))throw new Error();store.authorize({id,device_id:d,signature:sig,method:req.method,target:rawTarget,body:raw});const local=store.active(d);if(!local?.admin){if(local?.kind!=='coordinator'||!local.actor_active)throw new Error();if(req.method==='GET'&&pathOnly==='/v1/devices/me')return json(res,200,{device:{id:local.id,name:local.label,kind:local.kind,status:'active',actor_id:local.actor_id,admin:false},actor:{id:local.actor_id,name:local.actor_name||local.actor_id,kind:'human',active:true,admin:false}});const frozen=local.actor_id==='artem'&&local.id==='artems-macbook-pro'&&local.kind==='coordinator'&&local.actor_active&&!local.admin;if(writeRoute&&frozen)record(await delegatedCheck(writeRoute,raw),local.actor_id,local.id,null);else if(!isCoordinatorRead(req.method,pathOnly))throw new Error()}if(req.method==='POST'&&rawTarget==='/v1/pairing-codes'&&!local?.admin)throw new Error()}}
 ...
 }catch(e){const status=e.status||403,pairingMessage=/^pairing code is (?:invalid(?: or already used)?|already used|expired)$/.test(e.message)?e.message:null,message=status===413?`request body exceeds ${e.limit===8192?'8KiB':'1MiB'}`:status===502?'upstream unavailable':pairingMessage||'authorization failed';json(res,status,{error:{code:status===413?'body_too_large':status===502?'upstream_unavailable':'authorization_failed',message}})}};
