...
const VC_TICKET=/^\/v1\/tickets\/(VC-[1-5])\/state$/;
...
function delegatedWrite(method,rawTarget,pathOnly){
 ...
 if(method==='POST'&&pathOnly==='/v1/tickets')return{action:'create',ticket:null,valid:createTicketBody};
 if(method==='PATCH'&&(match=pathOnly.match(/^\/v1\/tickets\/([^/]+)$/))&&DELEGATED_TICKET_ID.test(match[1]))return{action:'edit',ticket:match[1],valid:editTicketBody};
 if(method==='POST'&&(match=pathOnly.match(/^\/v1\/tickets\/([^/]+)\/notes$/))&&DELEGATED_TICKET_ID.test(match[1]))return{action:'note',ticket:match[1],valid:noteBody};
 return null;
}
...
export async function createPhoneGateway({authDb,origin,upstream,upstreamAddressPolicy,upstreamAuthorization,cert,key,tlsTerminated=false,testMode=false,testHooks,now}={}){
 ...
 const handler=async(req,res)=>{secure(res);try{const rawTarget=requestTarget(req),pathOnly=rawTarget.split('?',1)[0];
  ...
  const raw=await body(req,1048576);let vcWrite=null,vcTicketWrite=null,payload=null;if(isApi){if(!upstreamAuthorization)throw new Error();if(sourceLimited(req))throw Object.assign(new Error(),{status:429});const match=rawTarget===pathOnly&&req.method==='POST'&&pathOnly.match(VC_TICKET),d=req.headers['x-viq-device'],id=req.headers['x-viq-challenge'],sig=req.headers['x-viq-signature'];const bearerMode=Boolean(match&&d===undefined&&id===undefined&&sig===undefined);if(bearerMode){const authorization=req.headers.authorization;if(typeof authorization!=='string'||authorization.length>519||!/^Bearer [\x21-\x7e]{1,512}$/.test(authorization))throw new Error();const data=parse(raw);if(!exactStateBody(data))throw new Error();const identity=await upstreamJson('/v1/devices/me',authorization.slice(7));if(identity?.actor?.id!=='artem'||identity.actor.active!==true||identity.actor.admin!==false||identity?.device?.id!=='artems-macbook-pro'||identity.device.kind!=='coordinator'||identity.device.status!=='active'||identity.device.admin!==false)throw new Error();const lookup=await upstreamJson(`/v1/tickets/${match[1]}`);if(lookup?.ticket?.id!==match[1]||lookup.ticket.project!=='VC')throw new Error();vcWrite={authMode:'bearer-delegation',ticket:match[1],state:data.state,device:'artems-macbook-pro'}
  }else{if(typeof d==='string'&&deviceLimited(d))throw Object.assign(new Error(),{status:429});if([d,id,sig].some(x=>typeof x!=='string'||x.length>512))throw new Error();store.authorize({id,device_id:d,signature:sig,method:req.method,target:rawTarget,body:raw});const local=store.active(d);if(!local?.admin){if(local?.kind!=='coordinator'||!local.actor_active)throw new Error();if(req.method==='GET'&&pathOnly==='/v1/devices/me')return json(res,200,{device:{id:local.id,name:local.label,kind:local.kind,status:'active',actor_id:local.actor_id,admin:false},actor:{id:local.actor_id,name:local.actor_name||local.actor_id,kind:'human',active:true,admin:false}});const frozen=local.actor_id==='artem'&&local.id==='artems-macbook-pro'&&local.kind==='coordinator'&&local.actor_active&&!local.admin,delegated=frozen?delegatedWrite(req.method,rawTarget,pathOnly):null;if(match&&frozen){const data=parse(raw);if(!exactStateBody(data))throw new Error();const lookup=await upstreamJson(`/v1/tickets/${match[1]}`);if(lookup?.ticket?.id!==match[1]||lookup.ticket.project!=='VC')throw new Error();vcWrite={ticket:match[1],state:data.state,device:local.id}}else if(delegated){const data=parse(raw);if(!delegated.valid(data))throw new Error();if(delegated.ticket!==null){const lookup=await upstreamJson(`/v1/tickets/${encodeURIComponent(delegated.ticket)}`);if(lookup?.ticket?.id!==delegated.ticket||lookup.ticket.project!==VC_PROJECT)throw new Error()}if(delegated.action==='note')payload=Buffer.from(JSON.stringify({...data,message:`${data.message} ${gatewayAttribution(local.actor_id,local.id)}`}));vcTicketWrite={actor:local.actor_id,device:local.id,action:delegated.action,detail:delegated.ticket===null?{project:VC_PROJECT,title:data.title}:{ticket_id:delegated.ticket}}}else if(!isCoordinatorRead(req.method,pathOnly))throw new Error()}if(req.method==='POST'&&rawTarget==='/v1/pairing-codes'&&!local?.admin)throw new Error()}}
