...
export async function createPhoneGateway({authDb,origin,upstream,upstreamAddressPolicy,cert,key,tlsTerminated=false,testMode=false,testHooks,now}={}){
 ...
 const hit=(key,max=120)=>{const t=Date.now();prune(t);const v=buckets.get(key)||{t,n:0};if(t-v.t>=WINDOW){v.t=t;v.n=0}v.n++;buckets.delete(key);buckets.set(key,v);return v.n>max};
 const sourceLimited=(req)=>hit(`ip:${req.socket.remoteAddress||'unknown'}`);
 const redeemLimited=(req)=>hit(`redeem:${req.socket.remoteAddress||'unknown'}`,5);
 const deviceLimited=(device)=>DEVICE_ID.test(device)&&hit(`device:${device}`);
 const authorize=(req,target,raw)=>{const d=req.headers['x-viq-device'],id=req.headers['x-viq-challenge'],sig=req.headers['x-viq-signature'];if([d,id,sig].some(x=>typeof x!=='string'||x.length>512))throw new Error();store.authorize({id,device_id:d,signature:sig,method:req.method,target,body:raw})};
 const handler=async(req,res)=>{secure(res);try{const rawTarget=requestTarget(req),pathOnly=rawTarget.split('?',1)[0];
  ...
  if(req.method==='POST'&&rawTarget==='/__phone/redeem'){if(redeemLimited(req))throw Object.assign(new Error(),{status:429});const data=parse(await body(req,8192));return json(res,200,store.redeemCode(data))}
  ...
  const management=rawTarget==='/__phone/devices'||rawTarget==='/__phone/codes'||/^\/__phone\/devices\/[A-Za-z0-9_-]{16,100}\/revoke$/.test(rawTarget);
  if(management){if(sourceLimited(req))throw Object.assign(new Error(),{status:429});const raw=await body(req,8192);authorize(req,rawTarget,raw);if(req.method==='GET'&&rawTarget==='/__phone/devices')return json(res,200,{devices:store.listDevices()});if(req.method==='POST'&&rawTarget==='/__phone/codes'){const data=parse(raw);return json(res,200,store.createCode(data.label))}if(req.method==='POST'&&rawTarget.endsWith('/revoke'))return json(res,200,{revoked:store.revokeDevice(rawTarget.split('/')[3])});throw Object.assign(new Error(),{status:404})}
  ...
  const raw=await body(req,1048576);if(isApi){if(sourceLimited(req))throw Object.assign(new Error(),{status:429});const d=req.headers['x-viq-device'];if(typeof d==='string'&&deviceLimited(d))throw Object.assign(new Error(),{status:429});authorize(req,rawTarget,raw)}
