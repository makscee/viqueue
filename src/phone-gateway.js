...
export async function createPhoneGateway({authDb,origin,upstream,upstreamAddressPolicy,upstreamAuthorization,cert,key,tlsTerminated=false,testMode=false,testHooks,now}={}){
 ...
 const handler=async(req,res)=>{secure(res);try{const rawTarget=requestTarget(req),pathOnly=rawTarget.split('?',1)[0];
  ...
  const raw=await body(req,1048576);let vcWrite=null;if(isApi){if(!upstreamAuthorization)throw new Error();if(sourceLimited(req))throw Object.assign(new Error(),{status:429});const d=req.headers['x-viq-device'];if(typeof d==='string'&&deviceLimited(d))throw Object.assign(new Error(),{status:429});const id=req.headers['x-viq-challenge'],sig=req.headers['x-viq-signature'];if([d,id,sig].some(x=>typeof x!=='string'||x.length>512))throw new Error();store.authorize({id,device_id:d,signature:sig,method:req.method,target:rawTarget,body:raw});const local=store.active(d);if(!local?.admin){if(local?.kind!=='coordinator'||!local.actor_active)throw new Error();if(req.method==='GET'&&pathOnly==='/v1/devices/me')return json(res,200,{device:{id:local.id,name:local.label,kind:local.kind,status:'active',actor_id:local.actor_id,admin:false},actor:{id:local.actor_id,name:local.actor_name||local.actor_id,kind:'human',active:true,admin:false}});const match=rawTarget===pathOnly&&req.method==='POST'&&pathOnly.match(VC_TICKET);if(match&&local.actor_id==='artem'&&local.id==='artems-macbook-pro'&&local.kind==='coordinator'&&local.actor_active&&!local.admin){const data=parse(raw);if(!exactStateBody(data))throw new Error();const lookup=await upstreamJson(`/v1/tickets/${match[1]}`);if(lookup?.ticket?.id!==match[1]||lookup.ticket.project!=='VC')throw new Error();vcWrite={ticket:match[1],state:data.state,device:local.id}}else if(!isCoordinatorRead(req.method,pathOnly))throw new Error()}if(req.method==='POST'&&rawTarget==='/v1/pairing-codes'&&!local?.admin)throw new Error()}
