...
evidence="${VIQ_EVIDENCE_DIR:-$work/evidence}"; mkdir -p "$evidence"; out="$evidence/mcp-e2e-output.txt"; : >"$out"; run(){ VIQ_SERVER="$base" VIQ_OPERATOR_TOKEN=secret node test/mcp-e2e-client.js "$@"; }
