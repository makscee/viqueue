...
{ run create; run ticket-create; run claim | jq 'del(.structuredContent.claim_token) | (.content[0].text = "[claim response redacted]")'; run events; echo MCP_E2E_OK; } >"$out" 2>&1; cat "$out"
