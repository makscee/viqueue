...
try {
  ...
  await page.locator('.machine-pair-form').getByLabel('Role').selectOption('Agent');
  await page.locator('.machine-pair-form').getByLabel('Name').fill('Disposable isolated machine');
  const issuanceResponse = page.waitForResponse((response) => response.url().endsWith('/v1/machines/pairing-codes')); await page.locator('.machine-pair-form').getByRole('button', { name: 'Generate one-time code' }).click();
  const issuedMachine = await (await issuanceResponse).json(); const disposableCode = await page.locator('.one-time-code').textContent();
  const disposableContext = await browser.newContext(); const disposablePage = await disposableContext.newPage(); await disposablePage.goto(base);
  await disposablePage.getByLabel('One-time code').fill(disposableCode); await disposablePage.getByLabel('Device ID').fill(issuedMachine.id); await disposablePage.getByLabel('Device name').fill('Disposable isolated machine'); const disposablePair = disposablePage.waitForResponse((response) => response.url().endsWith('/v1/devices/pair')); await disposablePage.getByRole('button', { name: 'Pair device' }).click();
  const disposableCredential = (await (await disposablePair).json()).credential; await disposablePage.getByText(/board requires an admin actor/i).waitFor(); assert.equal(await disposablePage.evaluate(() => localStorage.getItem('viq.deviceCredential')), null);
  await page.getByRole('button', { name: 'Clear code' }).click(); await page.getByRole('button', { name: 'Close' }).click(); await page.getByRole('button', { name: 'Refresh' }).click();
  await page.getByRole('button', { name: 'Machines', exact: true }).click(); const machineRow = page.locator('.machine-row', { hasText: 'Disposable isolated machine' }); await machineRow.getByRole('button', { name: 'Revoke' }).click(); await machineRow.getByRole('button', { name: 'Confirm revoke' }).click(); await page.getByText('Machine revoked').waitFor(); assert.equal(await page.locator('.machine-row', { hasText: 'Disposable isolated machine' }).count(), 0); assert.equal((await fetch(`${base}/v1/devices/me`, { headers: { authorization: `Bearer ${disposableCredential}` } })).status, 401); scenarios.machinesPairRevoke = true; await disposableContext.close();
