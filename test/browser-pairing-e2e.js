...
try {
  ...
  await page.getByRole('button', { name: 'Machines', exact: true }).click(); await page.locator('.machine-pair-form').getByLabel('Agent').check();
  await page.locator('.machine-pair-form').getByLabel('Name').fill('Disposable isolated machine');
  const issuanceResponse = page.waitForResponse((response) => response.url().endsWith('/v1/machines/pairing-codes')); await page.locator('.machine-pair-form').getByRole('button', { name: 'Create code' }).click();
  ...
  const assignment = page.locator('#modal-content select[name="assignment"]'); assert.deepEqual(await assignment.locator('option').allTextContents(), ['Unassigned', 'Human', 'Agent']); await assignment.selectOption('Agent');
  ...
  const created = (await (await createResponse).json()).ticket; assert.deepEqual({ id: created.id, assignment: created.assignment, worker: created.assigned_worker }, { id: 'DOG-1', assignment: 'Agent', worker: null });
  const createdCard = page.locator('.ticket-card[data-id="DOG-1"]'); await createdCard.waitFor(); await createdCard.getByText('Agent', { exact: true }).waitFor(); await page.getByText('DOG-1 created.').waitFor(); scenarios.genericAgentAssignmentVisible = true;
