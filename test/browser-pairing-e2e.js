...
try {
  ...
  await page.getByRole('button', { name: 'Done' }).click(); assert.equal(await page.locator('.one-time-code').count(), 0); assert.equal(await page.evaluate((code) => document.body.innerText.includes(code), disposableCode), false); await page.getByRole('button', { name: 'Refresh' }).click();
