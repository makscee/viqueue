...
try {
 ...
 const mobile=await browser.newContext({ignoreHTTPSErrors:true,viewport:{width:390,height:844}}),mp=await mobile.newPage(),mobileRequests=[];mp.on('request',r=>mobileRequests.push(r.url()));await mp.goto(origin);await mp.getByLabel('Six-digit pairing code').fill(code);await mp.getByLabel('Device label (optional)',{exact:true}).fill('Pocket');await mp.getByRole('button',{name:'Pair this device'}).click();await mp.locator('#phone-app').waitFor({state:'visible'});
 assert.equal(await page.evaluate(()=>fetch('/v1/actors').then(r=>r.status)),200);assert.equal(await mp.evaluate(()=>fetch('/v1/actors').then(r=>r.status)),200);await page.reload();await page.locator('#device-list li').filter({hasText:'Pocket'}).waitFor();assert.equal(await page.locator('#device-list li').count(),2);
 const repairCode=gateway.authStore.createCode('Pocket repaired').code;mp.once('dialog',dialog=>dialog.accept());await mp.locator('#device-list li').filter({hasText:'Pocket'}).getByRole('button',{name:'Revoke'}).click();await mp.locator('#phone-auth').waitFor({state:'visible'});await mp.getByLabel('Six-digit pairing code').fill(repairCode);await mp.getByLabel('Device label (optional)',{exact:true}).fill('Pocket repaired');await mp.getByRole('button',{name:'Pair this device'}).click();await mp.locator('#phone-app').waitFor({state:'visible'});assert.equal(await mp.evaluate(()=>fetch('/v1/actors').then(r=>r.status)),200);assert.equal(await page.evaluate(()=>fetch('/v1/actors').then(r=>r.status)),200);const codesBefore=mobileRequests.filter(url=>new URL(url).pathname==='/__phone/codes').length;await mp.getByLabel('New device label (optional)').fill('Post-repair device');await mp.getByRole('button',{name:'Add device'}).click();await mp.locator('#pairing-result').getByText(/expires in 10 minutes/).waitFor();await new Promise(resolve=>setTimeout(resolve,100));const codesAfter=mobileRequests.filter(url=>new URL(url).pathname==='/__phone/codes').length;assert.equal(codesAfter-codesBefore,1,'one post-repair Add device submit must create exactly one pairing-code request');
 const attacker=await browser.newContext({ignoreHTTPSErrors:true});await attacker.addCookies(await desktop.cookies());const attackerPage=await attacker.newPage();await attackerPage.goto(origin);await attackerPage.getByLabel('Six-digit pairing code').waitFor();assert.equal(await attackerPage.evaluate(()=>fetch('/v1/actors').then(r=>r.status)),403);
 ...
 await Promise.all([desktop.close(),mobile.close(),attacker.close()]);console.log('phone auth semantic E2E passed: legacy link, code redemption, confined keys, two active devices, self-revoke/re-pair, one post-repair Add-device request, first-device preservation, copied-cookie denial');
