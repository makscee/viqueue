...
try {
 ...
 const mobile=await browser.newContext({ignoreHTTPSErrors:true,viewport:{width:390,height:844}}),mp=await mobile.newPage();await mp.goto(origin);await mp.getByLabel('Six-digit pairing code').fill(code);await mp.getByLabel('Device label (optional)',{exact:true}).fill('Pocket');await mp.getByRole('button',{name:'Pair this device'}).click();await mp.locator('#phone-app').waitFor({state:'visible'});
 ...
 await Promise.all([desktop.close(),mobile.close(),attacker.close()]);console.log('phone auth semantic E2E passed: legacy link, code redemption, confined keys, two active devices, self-revoke/re-pair, first-device preservation, copied-cookie denial');
