...
const profile=path.join(work,'persistent-profile-a'),profileB=path.join(work,'persistent-profile-b'),profileC=path.join(work,'persistent-profile-c');
...
try{
  ...
  await submit(expired.code);await page.getByText(/expired/i).waitFor();clock=Date.now();
  ...
  await page.getByRole('button',{name:'Machines',exact:true}).click();await page.locator('.machine-pair-form').getByLabel('Name').fill('Secondary browser');await page.locator('.machine-pair-form').getByRole('button',{name:'Create code'}).click();const secondCode=await page.locator('.one-time-code').textContent();assert.equal(page.url().includes(secondCode),false);assert.equal(await page.evaluate(code=>JSON.stringify({...localStorage}).includes(code),secondCode),false);contextB=await chromium.launchPersistentContext(profileB,{headless:true,ignoreHTTPSErrors:true,viewport:{width:1280,height:900}});let pageB=contextB.pages()[0]||await contextB.newPage();observe(pageB);await pageB.goto(origin);await pageB.locator('#phone-auth').getByLabel('Pairing code').fill(secondCode);await pageB.getByRole('button',{name:'Pair browser'}).click();await pageB.locator('#app-shell').waitFor({state:'visible'});await page.getByRole('button',{name:'Done'}).click();assert.equal(await page.locator('.one-time-code').count(),0);assert.equal(await page.evaluate(code=>document.body.innerText.includes(code),secondCode),false);assert.equal(await pageB.evaluate(()=>fetch('/v1/devices/me').then(r=>r.status)),200);assert.equal(await pageB.evaluate(()=>fetch('/v1/projects').then(r=>r.status)),200);assert.equal(await page.evaluate(()=>fetch('/v1/devices/me').then(r=>r.status)),200);const secondId=gateway.authStore.db.prepare('SELECT id FROM devices WHERE label=? AND active=1').get('Secondary browser').id;
  ...
  gateway.authStore.revoke(secondId);await pageB.getByRole('button',{name:'Refresh'}).click();await pageB.getByRole('heading',{name:'Pair this browser'}).waitFor();await pageB.locator('#phone-auth').getByText(/revoked|no longer authorized/i).waitFor();assert.equal(await page.evaluate(()=>fetch('/v1/devices/me').then(r=>r.status)),200);assert.equal(await page.evaluate(()=>fetch('/v1/projects').then(r=>r.status)),200);
  await contextB.close();contextB=null;contextB=await chromium.launchPersistentContext(profileC,{headless:true,ignoreHTTPSErrors:true});pageB=contextB.pages()[0]||await contextB.newPage();observe(pageB);await pageB.goto(origin);await pageB.getByRole('heading',{name:'Pair this browser'}).waitFor();const replacement=gateway.authStore.createPair({deviceId:'browser_repaired_001',actorId:'browser-e2e-coordinator',admin:true,label:'Repaired browser'});await pageB.locator('#phone-auth').getByLabel('Pairing code').fill(replacement.code);await pageB.getByRole('button',{name:'Pair browser'}).click();await pageB.locator('#phone-app').waitFor({state:'visible'});await pageB.locator('#app-shell').waitFor({state:'visible'});assert.equal(await pageB.evaluate(()=>fetch('/v1/devices/me').then(response=>response.status)),200);await pageB.locator('#status').filter({hasText:/tickets shown|No projects yet/}).waitFor();assert.deepEqual(gateway.authStore.status().devices.map(d=>d.id).sort(),['browser_primary_0001','browser_repaired_001']);
  await assertSecretAbsent(page,[expired.code,fresh.code,secondCode,replacement.code]);
  assert.equal(networkEvidence.some(line=>/authorization:/i.test(line)),false);
  const audit=JSON.stringify(gateway.authStore.status().audit);assert.equal([expired.code,fresh.code,secondCode,replacement.code].some(code=>audit.includes(code)),false);
