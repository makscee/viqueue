...
try {
  ...
  const approval = page.locator('[data-surface="Activity"] .question-card').filter({ hasText: 'Approve submitted work?' }); await approval.waitFor(); assert.match(await approval.locator('small').textContent(), /Approval/); await approval.getByRole('button', { name: 'Accept work · Human Accept', exact: true }).click(); await page.getByText('Work accepted').waitFor();
