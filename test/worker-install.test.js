...
test('worker install atomically switches an immutable exact release and rolls back to 1398284e', async () => {
  ...
  for (const file of ['index.ts', 'worker-runtime.mjs', 'credential-store.mjs', 'vault-sync.mjs']) await writeFile(path.join(packageRoot, 'extensions', 'viq-worker', file), 'export {};\n');
