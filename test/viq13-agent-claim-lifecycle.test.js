...
test('VIQ-13 Store claims fail closed without pre-existing session authority and do not mutate tickets', async () => {
  ...
  for (const attempt of [
    ...
    () => store.claimNext({ project: 'AAA', device: a.device.id }),
    () => store.claim(ticket.id, { actor: a.device.id }),
    () => store.claimNext({ project: 'AAA', actor: a.device.id, session_capability: 'ps_chosen.not-server-issued' }),
    () => store.claim(ticket.id, { device: a.device.id, session_capability: wrongMachine.session_capability }),
    () => store.claimNext({ project: 'AAA', device: a.device.id, session_capability: revoked.session_capability })
  ]) {
...
test('VIQ-13 atomically claims authoritative first eligible Agent Open ticket and records machine/session provenance', async () => {
  ...
  const claim = await store.claimNext({ project: 'AAA', device: a.device.id, session_capability: sessionA.session_capability });
  assert.equal(claim.ticket.id, 'AAA-1');
...
test('VIQ-13 concurrent stores cannot share one project-scoped eligible ticket', async () => {
  const { store, file, a, b } = await fixture();
  ...
  const results = await Promise.all([store.claimNext({ project: 'AAA', device: a.device.id, session_capability: sessionA.session_capability }), other.claimNext({ project: 'AAA', device: b.device.id, session_capability: sessionB.session_capability })]);
  assert.equal(results.filter(Boolean).length, 1); assert.equal(results.find(Boolean).ticket.id, 'AAA-1');
  assert.equal((await store.getTicket('ZZZ-1')).claim, null);
...
test('VIQ-13 exact-session release reopens; stale and non-owning sessions cannot cross the fence', async () => {
  ...
  const first = await store.claimNext({ project: 'AAA', device: a.device.id, session_capability: sessionA.session_capability }); const firstAuth = authority(first, sessionA.session_capability);
...
test('VIQ-13 HTTP rejects chosen IDs, cross-session, cross-machine, and revoked capabilities', async (t) => {
  ...
  assert.equal((await call(a.credential, '/v1/tickets/claim-next', { session_id: 'pi-session-victim-00000001', project: 'AAA' })).body.error.code, 'client_session_authority_forbidden');
  ...
  const claim = (await call(a.credential, '/v1/tickets/claim-next', { project: 'AAA' }, sa.session_capability)).body; const auth = { claim_id: claim.ticket.claim.claim_id, claim_token: claim.claim_token, generation: claim.ticket.claim.generation };
