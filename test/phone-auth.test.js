...
test('additional pairing can be created while current device stays active', async()=>{ const f=await fixture(); try { assert.ok(f.store.createPair().url); const st=f.store.status(); assert.equal(st.paired,true); assert.ok(st.audit.some(x=>x.action==='paired')); } finally { await cleanup(f); } });
