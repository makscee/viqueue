...
export function viqHelp(status = {}) {
  ...
  return ['VIQ commands', '/viq pair <code>       Pair this Pi with a one-time code. Example: /viq pair <code>', '/viq status            Show pairing and worker state. Example: /viq status', '/viq poll              Poll generic Agent work across all projects. Example: /viq poll', '/viq claim <ticket>    Claim that exact eligible Open ticket for initial or requested-change work.', '/viq continue <ticket> Continue only that exact ticket after an answered blocking question.', '/viq stop              Stop polling safely. Example: /viq stop', 'viq_submit records an outcome summary and backend-neutral immutable evidence references produced elsewhere; it never publishes artifacts.', next].join('\n');
}

export function viqStatus(status = {}, baseUrl = '') {
  return [`VIQ is ${status.paired ? 'paired' : 'not paired'}; worker is ${status.mode || 'stopped'}.`, status.device ? `Machine: ${status.device}.` : null, status.project ? `Compatibility project filter: ${status.project}.` : 'Scope: generic Agent work across all projects.', status.ticket ? `Current ticket: ${status.ticket}.` : null, status.last_error ? `Last error: ${status.last_error}.` : null, `Target: ${baseUrl || 'not configured'}.`, status.paired ? (status.mode === 'stopped' ? 'Next: /viq poll' : 'Next: /viq stop when you are done.') : 'Next: /viq pair <code>'].filter(Boolean).join(' ');
}
