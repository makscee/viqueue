...
- Worker claims require the same active-device, assignment, state, blocker, and active-claim predicate through HTTP, CLI, MCP, and `/viq-worker`.
- Claim mutations additionally require the current claim ID, generation, and token.
...
The historical phone-browser pairing implementation is not the pairing-PoC identity store and must not be treated as an alternate authorization path. A future gateway must authenticate to the same small core device identity before use.
