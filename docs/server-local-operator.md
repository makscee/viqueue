...
The handler exposes exactly `POST /v1/operator/browser-pairings` on the Unix listener. It accepts exactly `{name}` and internally calls the same Store pairing primitive used by the authenticated coordinator API. It binds a generated Browser device ID, exact name, active Human admin actor, coordinator kind, and normal maximum TTL. It returns no bearer credential. Pair consumption still occurs through ordinary public `POST /v1/devices/pair`, preserving binding, expiry, kind, and single-use checks.
