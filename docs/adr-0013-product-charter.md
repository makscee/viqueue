...
**Viq is a minimal, transport-neutral, durable work-coordination layer through which humans and heterogeneous agents request, claim, discuss, hand off, and record completion, with optional compatibility submission/review; the Kanban board is a projection, not the authority.**
...
- record authorized worker completion and provenance without certifying correctness or publication;
- retain optional compatibility submission/review commands and record their decisions when an external policy chooses that route;
...
- certify an outcome as correct, reviewed, accepted for publication, or published merely because it records completion or a compatibility review decision.
...
4. **Completion.** The claim holder may complete work with an outcome plus immutable evidence references. Evidence is opaque to the kernel except for bounded validation and storage; it may identify commits, objects, reports, URLs, or another backend's immutable references. Completion ends claim authority and records the authorized worker's report and provenance. It does not certify correctness, review, publication readiness, or publication.
5. **Optional compatibility review.** Legacy submit/accept remains available for clients whose external workflow chooses it. The external runtime or publication system owns whether review is required, who may review, and whether self-review is prohibited. Viq records and applies a requested compatibility review transition; mandatory external/human acceptance and anti-self-approval are not kernel invariants.
...
| Complete outcome with immutable evidence references | Records authorized completion/provenance; does not certify or publish | **Owns outcome report** | Supplies immutable references | Optional external policy may require review |
| Decide correctness, publication readiness, or external acceptance | Does not decide; records a compatibility decision when asked | No | No | **Owned outside Viq according to runtime policy** |
...
| Later accepted refinement | Claims are durable fenced authority, not liveness; progress is observation; submission and review are separate. | `docs/adr-0008-v03-daily-alpha-core.md` and `docs/adr-0009-daily-alpha-actors-questions.md` at commits [`436cf5e`](https://github.com/makscee/viqueue/commit/436cf5e) and [`4ebc952`](https://github.com/makscee/viqueue/commit/4ebc952). | Retained where the compatibility review route is used; superseded where it implied every completion must enter review. | Silence and machine metadata must never be treated as runtime health. |
| Latest accepted boundary | A claim holder may record direct completion and provenance. Viq does not certify correctness or publication; external runtimes own review requirements and separation-of-duties policy. | Director-approved VIQ CLI adoption boundary, 2026-09-07. | Canonical. | Supersedes only mandatory-review and anti-self-approval claims in this charter and earlier lifecycle descriptions. Legacy submit/accept remains optional compatibility behavior. |
| Superseded refinement | Pairing and role assignment once constrained claim eligibility through exact assignments. | `README.md` and `docs/adr-0012-pairing-poc.md` at commit [`5a68920`](https://github.com/makscee/viqueue/commit/5a68920). | Superseded: supported creation is categorical; any active paired Agent machine may atomically claim generic Agent work. | “Launch authorization” means permission to request a claim, not authority or ability to launch a worker. |
...
- direct completion is the ordinary kernel path; legacy submit/accept remains optional compatibility behavior rather than a required review invariant. This charter does not claim any deployment or live-state change.
