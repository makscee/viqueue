...
4. Provision an operator-controlled file containing only the dedicated upstream coordinator credential, mode `0600`, and pass `--upstream-authorization-file=/APPROVED/PATH/upstream.credential`. The gateway never accepts browser credentials as upstream authority and fails every `/v1/*` request before upstream if this option is absent.
...
6. Run `viq-phone-auth pair-create --db=… --origin=https://…`. Do not paste its URL into logs or tickets.
7. On the phone, open the fragment URL and tap **Pair this phone**. That is the single minimal phone action after the cutover gate.
8. Inspect `viq-phone-auth status --db=… --origin=https://…` (`--json` is available). If a key/profile is lost or access must end, run `revoke`; create a new pair only afterward.
