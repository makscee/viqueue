import { activityFact, applyActivityFilters, applyTicketFilters, createModalController, dedupeTickets, reconcileProjectSelection, renderMarkdown, reviewSourceFacts } from './ui-core.js';
...
function chip(label, pressed, action) { const button = document.createElement('button'); button.type = 'button'; button.className = 'filter-chip'; const mark = Object.assign(document.createElement('span'), { className: 'selection-mark', textContent: '✓' }); mark.setAttribute('aria-hidden', 'true'); button.append(mark, label); button.setAttribute('aria-pressed', String(pressed)); button.addEventListener('click', action); return button; }
...
function safeReviewUri(value){try{const url=new URL(value);return ['https:','http:','urn:','file:'].includes(url.protocol)&&!url.username&&!url.password?url.href:null}catch{return null}}
function reviewBundleElement(bundle,ticket){const section=document.createElement('section');section.className='review-bundle';section.innerHTML='<h4>Review Bundle</h4>';section.append(markdownElement('div',bundle.summary,'markdown'));const addList=(title,items,render)=>{const h=document.createElement('h5');h.textContent=title;const ul=document.createElement('ul');items.forEach(item=>ul.append(render(item)));section.append(h,ul)};addList('Evidence',bundle.evidence??[],item=>{const li=document.createElement('li'),uri=safeReviewUri(item.uri),link=document.createElement(uri?.startsWith('http')?'a':'span');link.textContent=`${item.label} · ${item.kind}${item.digest?` · ${item.digest}`:''}`;if(link.tagName==='A'){link.href=uri;link.target='_blank';link.rel='noreferrer noopener'}li.append(link);return li});if(bundle.preview_url){const a=document.createElement('a');a.href=safeReviewUri(bundle.preview_url)??'#';a.textContent='Open preview';a.target='_blank';a.rel='noreferrer noopener';section.append(a)}for(const shot of bundle.screenshots??[]){const uri=safeReviewUri(shot.uri);if(!uri)continue;const figure=document.createElement('figure'),a=document.createElement('a');a.href=uri;a.target='_blank';a.rel='noreferrer noopener';a.textContent=`Screenshot: ${shot.label}`;figure.append(a);if(uri.startsWith('http')){const img=document.createElement('img');img.src=uri;img.alt=shot.label;img.loading='lazy';img.referrerPolicy='no-referrer';a.replaceChildren(img)}section.append(figure)}addList('Verification steps',bundle.verification_steps??[],item=>Object.assign(document.createElement('li'),{textContent:item}));addList('Tests',bundle.tests??[],item=>Object.assign(document.createElement('li'),{textContent:`${item.status.toUpperCase()} · ${item.name}`}));addList('Caveats',bundle.caveats?.length?bundle.caveats:['None reported.'],item=>Object.assign(document.createElement('li'),{textContent:item}));addList('Lifecycle facts',reviewSourceFacts(bundle,ticket),fact=>{const li=document.createElement('li'),value=document.createElement(fact.href?'a':'span');value.textContent=`${fact.label}: ${fact.value}`;if(fact.href){value.href=fact.href;value.target='_blank';value.rel='noreferrer noopener'}li.append(value);return li});const boundary=document.createElement('p');boundary.className='release-boundary';boundary.textContent='References and lifecycle facts only. VIQ does not review, merge, release, deploy, or verify production.';section.append(boundary);const visual=Boolean(bundle.preview_url||bundle.screenshots?.length);if((bundle.ui_change||ticket?.review_proof_class==='visual')&&!visual)section.append(Object.assign(document.createElement('p'),{className:'proof-warning',textContent:'Visual proof is absent. Human Accept requires explicit acknowledgement.'}));return section}
function questionCard(question, { compact = false, afterAnswer = null } = {}) {
  ...
  const submitted=question.kind==='approval'?[...state.events].reverse().find(event=>event.ticket_id===question.ticket_id&&event.type==='submitted'):null,bundle=submitted?.metadata?.review_bundle;if(bundle)article.append(reviewBundleElement(bundle,ticket));
  ...
  else {form.innerHTML = '<label>Note (optional)<textarea name="note" rows="2"></textarea></label><div class="form-actions"><button name="decision" value="accept">Accept work · Human Accept</button><button class="secondary" name="decision" value="request_changes">Request changes</button></div>';if(bundle&&(bundle.ui_change||ticket?.review_proof_class==='visual')&&!bundle.preview_url&&!bundle.screenshots?.length){const label=document.createElement('label');label.className='proof-ack';label.innerHTML='<input type="checkbox" name="proof_acknowledged"> I reviewed the result and explicitly acknowledge that visual proof is absent.';form.querySelector('.form-actions').before(label)}}
  form.addEventListener('submit', safely(async (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(form)); const decision = event.submitter?.value; [...form.elements].forEach((element) => { element.disabled = true; }); await request(`/v1/tickets/${encodeURIComponent(question.ticket_id)}/questions/${encodeURIComponent(question.id)}/answer`, { method: 'POST', body: JSON.stringify({ request_id: requestId, ...(decision ? { decision, note: data.note,proof_acknowledged:data.proof_acknowledged==='on' } : { answer: data.answer }) }) }); await refresh(); announce(decision === 'accept' ? 'Work accepted' : decision === 'request_changes' ? 'Changes requested' : 'Answer sent'); await afterAnswer?.(); }));
...
function eventItem(event) {
  const labels = { ticket_created: 'Ticket created', ticket_edited: 'Ticket edited', manual_event: 'Factual event', progress: 'Factual event', question_asked: 'Question asked', question_answered: 'Question answered', claimed: 'Work claimed', released: 'Claim released', submitted: 'Submitted', accepted: 'Approved', changes_requested: 'Changes requested', reopened: 'Reopened', state_changed: 'State changed', blocked: 'Blocked', block_resolved: 'Block resolved', review_recorded: 'Review recorded', merge_recorded: 'Merge recorded', release_recorded: 'Release recorded', production_verified: 'Production verified' };
...
async function showDetail(id, trigger, intent = modal.begin()) {
  ...
  const latestBundle=[...history.events].reverse().find(event=>event.type==='submitted')?.metadata?.review_bundle;if(latestBundle)content.append(reviewBundleElement(latestBundle,ticket));
...
async function openMachines(trigger) {
  ...
  for (const machine of state.machines) {
    ...
    const identity = document.createElement('span'); identity.innerHTML = '<strong></strong><small></small><small class="worker-presence"></small>'; identity.querySelector('strong').textContent = machine.name; identity.querySelector('small').textContent = `${machine.role} · ${machine.id}`; const presence=identity.querySelector('.worker-presence'); if(machine.role==='Agent'){const workerAt=machine.worker_heartbeat_at?new Date(machine.worker_heartbeat_at).toLocaleString():'never'; const ticketAt=machine.ticket_heartbeat_at?new Date(machine.ticket_heartbeat_at).toLocaleString():'never'; presence.textContent=`${machine.state??'stale/offline'} · worker heartbeat ${workerAt}${machine.ticket_id?` · ticket ${machine.ticket_id} heartbeat ${ticketAt}`:''}`; if(machine.ticket_id){const link=Object.assign(document.createElement('a'),{href:`#ticket-${machine.ticket_id}`,textContent:`Open ${machine.ticket_id}`});link.addEventListener('click',event=>{event.preventDefault();showDetail(machine.ticket_id,trigger)});presence.append(' · ',link)}}
  ...
  const pairing = document.createElement('form'); pairing.className = 'modal-form machine-pair-form'; pairing.innerHTML = '<h3>Pair device</h3><fieldset class="role-selector"><legend>Role</legend><label class="role-choice"><input type="radio" name="role" value="Human" checked><span class="role-choice-label"><span class="role-choice-check" aria-hidden="true">✓</span>Human</span></label><label class="role-choice"><input type="radio" name="role" value="Agent"><span class="role-choice-label"><span class="role-choice-check" aria-hidden="true">✓</span>Agent</span></label></fieldset><label>Name<input name="name" required maxlength="100" autocomplete="off"></label><button>Create code</button><div class="pairing-code-result" aria-live="polite"></div>';
